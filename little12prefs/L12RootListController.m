...
- (instancetype)init {
    ...
    if (self) {
        ...
        self.iconView.image       = [UIImage
            imageWithContentsOfFile:ROOT_PATH_NS(@"/Library/PreferenceBundles/little12prefs.bundle/icon@3x.png")];
        ...
        headerImageView.image        = [UIImage
            imageWithContentsOfFile:ROOT_PATH_NS(@"/Library/PreferenceBundles/little12prefs.bundle/Banner.png")];
...
- (NSArray *)specifiers {
    ...
    if (_specifiers == nil) {
        ...
        NSDictionary const *prefs = [[NSDictionary alloc]
            initWithContentsOfFile:ROOT_PATH_NS(@"/var/mobile/Library/Preferences/com.ryannair05.little12.plist")];
        ...
        if (firstTime) {
            ...
            [welcomeController
                addBulletedListItemWithTitle:@"App Support"
                                 description:@"The majority of apps work with Little12 without an issue. "
                                             @"For the exceptions, I'm doing my best to fix them though "
                                             @"sometimes it can prove to be a challenge. Turning on or off "
                                             @"the compatibility mode and/or device spoofing settings may "
                                             @"improve or worsen certain apps. "
                                       image:[UIImage systemImageNamed:@"2.circle.fill"]];
            [welcomeController addBulletedListItemWithTitle:@"Support"
                                                description:@"Little12 is made to be the best possible, but there "
                                                            @"are still some issues unfortunately. To report "
                                                            @"issues and for support I can be contacted over "
                                                            @"Email, Discord, Twitter, Reddit, and Github."
                                                      image:[UIImage systemImageNamed:@"3.circle.fill"]];
            [welcomeController addBulletedListItemWithTitle:@"Open Source"
                                                description:@"Little12 is open source and can be found on Github. "
                                                            @"Feel free to check out the code anytime and even "
                                                            @"make a pull request."
                                                      image:[UIImage systemImageNamed:@"4.circle.fill"]];

            OBBoldTrayButton *continueButton = [OBBoldTrayButton buttonWithType:1];
            [continueButton addTarget:self
                               action:@selector(dismissWelcomeController)
            ...
            welcomeController.buttonTray.effectView.effect =
...
- (void)dismissWelcomeController {
    ...
    NSMutableDictionary const *prefs = [[NSMutableDictionary alloc]
        initWithContentsOfFile:ROOT_PATH_NS(@"/var/mobile/Library/Preferences/com.ryannair05.little12.plist")];

    [prefs setValue:[NSNumber numberWithBool:NO] forKey:@"firstTime"];

    [prefs writeToFile:ROOT_PATH_NS(@"/var/mobile/Library/Preferences/com.ryannair05.little12.plist") atomically:YES];
}
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    [[UISwitch appearanceWhenContainedInInstancesOfClasses:@[ self.class ]] setOnTintColor:[UIColor colorWithRed:0.00
                                                                                                           green:0.51
                                                                                                            blue:1.00
                                                                                                           alpha:1.00]];
    [[UISlider appearanceWhenContainedInInstancesOfClasses:@[ self.class ]] setTintColor:[UIColor colorWithRed:0.00
                                                                                                         green:0.51
                                                                                                          blue:1.00
                                                                                                         alpha:1.00]];
}
...
- (void)viewDidLoad {
    ...
    if ([[NSFileManager defaultManager]
            fileExistsAtPath:ROOT_PATH_NS(@"/Library/MobileSubstrate/DynamicLibraries/KeyboardPlus.dylib")]) {
    ...
    //     [self removeContiguousSpecifiers:@[self.savedSpecifiers[@"numRecAppsonDock"]]
    //     animated:NO];
    // }
}
...
- (void)reloadSpecifiers {
    ...
    if (([[prefs objectForKey:@"iPadDock"] boolValue]) == 0) {
        ...
        [self removeContiguousSpecifiers:@[ self.savedSpecifiers[@"dockInSwitcher"] ] animated:NO];
        [self removeContiguousSpecifiers:@[ self.savedSpecifiers[@"recApponDock"] ] animated:NO];
        // [self removeContiguousSpecifiers:@[self.savedSpecifiers[@"roundedAppSwitcherNoDock"]]
        // animated:NO];
    } else if (([[prefs objectForKey:@"recApponDock"] boolValue]) == 0) {
        [self removeContiguousSpecifiers:@[ self.savedSpecifiers[@"numRecAppsonDock"] ] animated:NO];
    }
...
- (NSMutableArray *)appSpecifiers {
    ...
    for (NSString *bundleIdentifier in dataSourceUser.allKeys) {
        ...
        PSSpecifier *spe = [PSSpecifier preferenceSpecifierNamed:displayName
                                                          target:self
                                                             set:nil
                                                             get:@selector(getIsWidgetSetForSpecifier:)
                                                          detail:[L12AppSettingsController class]
                                                            cell:PSLinkListCell
                                                            edit:nil];
...
- (void)setPreferenceValue:(id)value specifier:(PSSpecifier *)specifier {
    ...
    if ([key isEqualToString:@"iPadDock"]) {
        if ([value boolValue] == false) {
            ...
            if (![self containsSpecifier:self.savedSpecifiers[@"iPadMultitasking"]]) {
                ...
                if (![self containsSpecifier:self.savedSpecifiers[@"iPadDockNumIcons"]]) {
                    [self insertContiguousSpecifiers:@[ self.savedSpecifiers[@"iPadDockNumIcons"] ]
                                    afterSpecifierID:@"iPadSwitcher"
                                            animated:YES];
                    [self insertContiguousSpecifiers:@[ self.savedSpecifiers[@"numAppsonDock"] ]
                                    afterSpecifierID:@"iPadDockNumIcons"
                                            animated:YES];
                    [self insertContiguousSpecifiers:@[ self.savedSpecifiers[@"recApponDock"] ]
                                    afterSpecifierID:@"numAppsonDock"
                                            animated:YES];
                    [self insertContiguousSpecifiers:@[ self.savedSpecifiers[@"dockInApps"] ]
                                    afterSpecifierID:@"recApponDock"
                                            animated:YES];
                    [self insertContiguousSpecifiers:@[ self.savedSpecifiers[@"dockInSwitcher"] ]
                                    afterSpecifierID:@"dockInApps"
                                            animated:YES];
                }

                if ([[prefs objectForKey:@"recApponDock"] boolValue]) {
                    [self insertContiguousSpecifiers:@[ self.savedSpecifiers[@"numRecAppsonDock"] ]
                                    afterSpecifierID:@"recApponDock"
                                            animated:YES];
                }

                [self insertContiguousSpecifiers:@[ self.savedSpecifiers[@"iPadMultitasking"] ]
                                afterSpecifierID:@"dockInSwitcher"
                                        animated:YES];
            }
        }
        ...
        if ([value boolValue] == false) {
            ...
            [self insertContiguousSpecifiers:@[ self.savedSpecifiers[@"keyboardSpacing"] ]
                            afterSpecifierID:@"keyboardDock"
                                    animated:YES];
        }
        ...
        if ([value boolValue] == false) {
            ...
            if ([self containsSpecifier:self.savedSpecifiers[@"roundedAppSwitcher"]])
                [self insertContiguousSpecifiers:@[ self.savedSpecifiers[@"appswitcherRoundness"] ]
                                afterSpecifierID:@"roundedAppSwitcher"
                                        animated:YES];
            else
                [self insertContiguousSpecifiers:@[ self.savedSpecifiers[@"appswitcherRoundness"] ]
                                afterSpecifierID:@"roundedAppSwitcherNoDock"
                                        animated:YES];
        }
        ...
        if ([value boolValue] == false) {
            ...
            [self insertContiguousSpecifiers:@[ self.savedSpecifiers[@"screenRoundness"] ]
                            afterSpecifierID:@"roundedCorners"
                                    animated:YES];
        }
    }

    if (([[prefs objectForKey:@"statusBarStyle"] integerValue] > 1) ||
        [[prefs objectForKey:@"bottomInset"] boolValue]) {
        if (![self containsSpecifier:self.savedSpecifiers[@"compatabilityMode"]]) {
            if ([self containsSpecifier:self.savedSpecifiers[@"screenRoundness"]])
                [self insertContiguousSpecifiers:@[ self.savedSpecifiers[@"compatabilityMode"] ]
                                afterSpecifierID:@"screenRoundness"
                                        animated:YES];
            else
                [self insertContiguousSpecifiers:@[ self.savedSpecifiers[@"compatabilityMode"] ]
                                afterSpecifierID:@"roundedCorners"
            ...
            [self insertContiguousSpecifiers:@[ self.savedSpecifiers[@"deviceSpoofing"] ]
                            afterSpecifierID:@"compatabilityMode"
                                    animated:YES];
        }
    }
...
- (instancetype)initWithStyle:(UITableViewCellStyle)style
    ...
    if (self) {
        ...
        // stringWithFormat:@"/var/jb/Library/PreferenceBundles/little11prefs.bundle/%@.png",
        // _user]];
        ...
        dispatch_async(dispatch_get_global_queue(0, 0), ^{
          ...
          NSURLRequest *request     = [NSURLRequest
              requestWithURL:[NSURL URLWithString:[NSString
                                                      stringWithFormat:@"https://pbs.twimg.com/profile_images/"
                                                                           @"1161080936836018176/4GUKuGlb_200x200.jpg"]]];
...
+ (NSURL *)_urlForUsername:(NSString *)user {
    ...
    /*    if ([[UIApplication sharedApplication] canOpenURL:[NSURL URLWithString:@"aphelion://"]]) {
            return [NSString stringWithFormat: @"aphelion://profile/%@", user]; // Easter egg by
       hbkirb } else*/
...
- (void)setSelected:(BOOL)arg1 animated:(BOOL)arg2 {
    if (arg1)
        [[UIApplication sharedApplication] openURL:[self.class _urlForUsername:_user]
                                           options:@{}
